...
class PeerConnectionV2 extends StateMachine {
    ...
    Object.defineProperties(this, {
      ...
      _iceGatheringTimeout: {
        value: new options.Timeout(
          () => this._handleIceGatheringResult(),
          DEFAULT_ICE_GATHERING_TIMEOUT_MS,
          false)
      },
      _iceRestartBackoff: {
  ...
  _handleIceConnectionStateChange() {
    ...
    if (this._lastIceConnectionState !== 'failed' && iceConnectionState === 'failed' && !this._shouldRestartIce && !this._isRestartingIce) {
      ...
      this._log.warn('ICE failed');
      this._iceRestartBackoff.backoff();
    } else if (['disconnected', 'failed'].includes(this._lastIceConnectionState) && ['connected', 'complete'].includes(iceConnectionState)) {
  ...
  /**
   * Handle the results of ICE gathering process after it is either
   * complete or has timed out.
   * @private
   * @returns {void}
   */
  _handleIceGatheringResult() {
    const log = this._log;
    const { iceConnectionState } = this._peerConnection;
    const iceFailedOrInactive = iceConnectionState === 'failed' || this._isIceConnectionInactive;
    // NOTE(mmalavalli): We schedule an ICE restart only if no local candidates were
    // generated AND ICE connection state has not yet failed or become inactive. If
    // ICE has already failed, or is already inactive, then an ICE restart has already
    // been scheduled, in which case we don't need to schedule it here.
    if (!this._didGenerateLocalCandidates) {
      log.warn('ICE failed to gather any local candidates');
      this._iceGatheringFailed = true;
      this.emit('iceConnectionStateChanged');
      this.emit('connectionStateChanged');
      if (!iceFailedOrInactive) {
        log.warn('An ICE restart will be scheduled');
        this._iceRestartBackoff.backoff();
      }
    }
  }
  ...
  _handleIceGatheringStateChange() {
    ...
    this._log.info(`ICE gathering state is "${iceGatheringState}"`);
  ...
  _initiateIceRestart() {
    ...
    this._log.warn('Attempting to restart ICE');
    this._shouldRestartIce = true;
    this.offer();
  }
